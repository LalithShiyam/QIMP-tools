...
class InitParser(object):
    def __init__(self):
        ...
        # train setting
        self.increase_factor_data = 3                                                 # 'Increase the data number passed each epoch'
        self.resample = True                                                          # 'Decide or not to rescale the images to a new resolution'
        self.new_resolution = (0.520833*1.3, 0.520833*1.3, 1*1.3)                     # 'New resolution'
        self.patch_size = [128, 128, 64]                                              # "Input dimension for the Unet3D"
        self.drop_ratio = 0.2                                                           # "Probability to drop a cropped area if the label is empty. All empty patches will be dropped for 0 and accept all cropped patches if set to 1"
        self.min_pixel = 0.1                                                          # "Percentage of minimum non-zero pixels in the cropped label"
        self.batch_size = 4                                                           # 'Batch size: the greater more GPUs are used and faster is the training'
